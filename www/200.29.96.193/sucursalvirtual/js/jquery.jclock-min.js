(function(A){A.fn.jclock=function(C){var B="0.2.1";
...
$this.diasES=["Domingo","Lunes","Martes","Miércoles","Jueves","Viernes","Sábado"];
...
$this.css({fontFamily:F.fontFamily,fontSize:F.fontSize,backgroundColor:F.background,color:F.foreground});
A.fn.jclock.startClock($this)
})
};
A.fn.jclock.startClock=function(B){A.fn.jclock.stopClock(B);
A.fn.jclock.displayTime(B)
};
A.fn.jclock.stopClock=function(B){if(B.running){clearTimeout(B.timerID)
}B.running=false
};
A.fn.jclock.displayTime=function(B){var C=A.fn.jclock.getTime(B);
B.html(C);
B.timerID=setTimeout(function(){A.fn.jclock.displayTime(B)
},1000)
};
A.fn.jclock.getDelay=function(){return($this.secondCounter++)*1000
};
A.fn.jclock.formatTime=function(D,C,B){var E=$this.format.replace("$anio$",C.getFullYear());
E=E.replace("$dia$",C.getDate());
E=E.replace("$hhmmss$",D);
E=E.replace("$ampm$",B);
if($this.language=="en"){E=E.replace("$nombreDia$",$this.diasEN[C.getDay()]);
E=E.replace("$nombreMes$",$this.mesesEN[C.getMonth()])
}else{E=E.replace("$nombreDia$",$this.diasES[C.getDay()]);
E=E.replace("$nombreMes$",$this.mesesES[C.getMonth()])
}return E
};
A.fn.jclock.getTime=function(C){var B=new Date();
var G,D,H;
var F=$this.serverTime.getTime()+A.fn.jclock.getDelay();
B.setTime(F);
if(C.utc==true){if(C.utc_offset!=0){B.setUTCHours(B.getUTCHours()+C.utc_offset)
}G=B.getUTCHours();
D=B.getUTCMinutes();
H=B.getUTCSeconds()
}else{G=B.getHours();
...
(G>=12)?J=" PM":J=" AM";
if(C.timeNotation=="12h"){G=((G>12)?G-12:G)
}else{G=((G<10)?"0":"")+G
}D=((D<10)?"0":"")+D;
H=((H<10)?"0":"")+H;
var E=G+":"+D+":"+H;
if((C.timeNotation!="12h")||(C.am_pm!=true)){J=""
}var K=$this.mesesES[B.getMonth()];
var I=$this.diasES[B.getDay()];
E=A.fn.jclock.formatTime(E,B,J);
return E
};
A.fn.jclock.defaults={timeNotation:"24h",am_pm:false,utc:false,fontFamily:"",fontSize:"",foreground:"",background:"",utc_offset:0,h_hour:""}
})(jQuery);