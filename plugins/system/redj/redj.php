  ...
  private static function getMacros(&$macropatterns, &$macroreplacements)
  {
    ...
    */
    ...
    if (!isset($patterns)) {
      ...
      $patterns[10] = "/\{querybuild ([^\}]+)\}/seuU";
      $patterns[11] = "/\{querybuildfull ([^\}]+)\}/seuU";
      $patterns[12] = "/\{querybuildappend ([^\}]+)\}/seuU";
      $patterns[16] = "/\{queryvar ([^\}]+)\}/seuU";
      $patterns[17] = "/\{queryvar ([^\}]+),([^\}]+)\}/seuU";
      $patterns[18] = "/\{requestvar ([^\}\,]+),?\}/seuU";
      $patterns[19] = "/\{requestvar ([^\}]+),([^\}]+)\}/seuU";
      $patterns[20] = "/\{authority\}/";
      $patterns[21] = "/\{baseonly\}/";
      $patterns[22] = "/\{pathfrombase\}/";
      $patterns[23] = "/\{pathltrim ([^\}]+)\}/seuU";
      $patterns[24] = "/\{pathrtrim ([^\}]+)\}/seuU";
      $patterns[29] = "/\{routeurl\}/e";
      $patterns[30] = "/\{routeurl ([^\}]+)\}/seuU";
      // Site macros
      $patterns[31] = "/\{sitename\}/";
      $patterns[32] = "/\{globaldescription\}/";
      $patterns[33] = "/\{globalkeywords\}/";
      $patterns[34] = "/\{username\}/e";
      $patterns[35] = "/\{username ([^\}]+)\}/seuU";
      $patterns[36] = "/\{userid\}/e";
      $patterns[37] = "/\{userid ([^\}]+)\}/seuU";
      // Database macros
      $patterns[38] = "/\{tableselect ([^\}]+)\}(.*)\{\/tableselect\}/seuU";
      // String macros
      $patterns[41] = "/\{substr ([^\}]+)\}(.*)\{\/substr\}/seuU";
      $patterns[42] = "/\{strip_tags\}(.*)\{\/strip_tags\}/seuU";
      $patterns[43] = "/\{extract ([^\}]+)\}(.*)\{\/extract\}/seuU";
      $patterns[44] = "/\{extractp ([^\}]+)\}(.*)\{\/extractp\}/seuU";
      $patterns[45] = "/\{extractdiv ([^\}]+)\}(.*)\{\/extractdiv\}/seuU";
      $patterns[47] = "/\{preg_placeholder ([^\}]+)\}(.*)\{\/preg_placeholder\}/seuU";
      $patterns[48] = "/\{lowercase\}(.*)\{\/lowercase\}/seuU";
      $patterns[49] = "/\{uppercase\}(.*)\{\/uppercase\}/seuU";
      $patterns[50] = "/\{urldecode\}(.*)\{\/urldecode\}/seuU";
      $patterns[51] = "/\{urlencode\}(.*)\{\/urlencode\}/seuU";
      $patterns[52] = "/\{rawurldecode\}(.*)\{\/rawurldecode\}/seuU";
      $patterns[53] = "/\{rawurlencode\}(.*)\{\/rawurlencode\}/seuU";
      $patterns[54] = "/\{str_replace ([^\}]+)\}(.*)\{\/str_replace\}/seuU";
    }
    ...
    if (!isset($replacements))
    {
      ...
      $replacements[10] = "plgSystemReDJ::manageMacroParams('querybuild', array(0 => '\\1'))";
      $replacements[11] = "plgSystemReDJ::manageMacroParams('querybuildfull', array(0 => '\\1'))";
      $replacements[12] = "plgSystemReDJ::manageMacroParams('querybuildappend', array(0 => '\\1'))";
      $replacements[16] = "plgSystemReDJ::manageMacroParams('queryvar', array(0 => '\\1'))";
      $replacements[17] = "plgSystemReDJ::manageMacroParams('queryvar', array(0 => '\\1', 1 => '\\2'))";
      $replacements[18] = "plgSystemReDJ::manageMacroParams('requestvar', array(0 => '\\1'))";
      $replacements[19] = "plgSystemReDJ::manageMacroParams('requestvar', array(0 => '\\1', 1 => '\\2'))";
      $replacements[20] = $uri_visited_full_url_parts['authority'];
      $replacements[21] = $uri_visited_full_url_paths['baseonly'];
      $replacements[22] = $uri_visited_full_url_paths['pathfrombase'];
      $replacements[23] = "plgSystemReDJ::manageMacroParams('pathltrim', array(0 => '\\1'))";
      $replacements[24] = "plgSystemReDJ::manageMacroParams('pathrtrim', array(0 => '\\1'))";
      $replacements[29] = "plgSystemReDJ::manageMacroParams('routeurl', array())";
      $replacements[30] = "plgSystemReDJ::manageMacroParams('routeurl', array(0 => '\\1'))";
      // Site macros
      $replacements[31] = $global_info['sitename'];
      $replacements[32] = $global_info['MetaDesc'];
      $replacements[33] = $global_info['MetaKeys'];
      $replacements[34] = "plgSystemReDJ::manageMacroParams('username', array(0 => ''))";
      $replacements[35] = "plgSystemReDJ::manageMacroParams('username', array(0 => '\\1'))";
      $replacements[36] = "plgSystemReDJ::manageMacroParams('userid', array(0 => ''))";
      $replacements[37] = "plgSystemReDJ::manageMacroParams('userid', array(0 => '\\1'))";
      // Database macros
      $replacements[38] = "plgSystemReDJ::manageMacroParams('tableselect', array(0 => '\\1', 1 => '\\2'))";
      // String macros
      $replacements[41] = "plgSystemReDJ::manageMacroParams('substr', array(0 => '\\1', 1 => '\\2'))";
      $replacements[42] = "plgSystemReDJ::manageMacroParams('strip_tags', array(0 => '\\1'))";
      $replacements[43] = "plgSystemReDJ::manageMacroParams('extract', array(0 => '\\1', 1 => '\\2'))";
      $replacements[44] = "plgSystemReDJ::manageMacroParams('extractp', array(0 => '\\1', 1 => '\\2'))";
      $replacements[45] = "plgSystemReDJ::manageMacroParams('extractdiv', array(0 => '\\1', 1 => '\\2'))";
      $replacements[47] = "plgSystemReDJ::manageMacroParams('preg_placeholder', array(0 => '\\1', 1 => '\\2'))";
      $replacements[48] = "plgSystemReDJ::manageMacroParams('lowercase', array(0 => '\\1'))";
      $replacements[49] = "plgSystemReDJ::manageMacroParams('uppercase', array(0 => '\\1'))";
      $replacements[50] = "plgSystemTagMeta::manageMacroParams('urldecode', array(0 => '\\1'))";
      $replacements[51] = "plgSystemTagMeta::manageMacroParams('urlencode', array(0 => '\\1'))";
      $replacements[52] = "plgSystemTagMeta::manageMacroParams('rawurldecode', array(0 => '\\1'))";
      $replacements[53] = "plgSystemTagMeta::manageMacroParams('rawurlencode', array(0 => '\\1'))";
      $replacements[54] = "plgSystemTagMeta::manageMacroParams('str_replace', array(0 => '\\1', 1 => '\\2'))";
    }
  ...
  private static function manageMacroParams($macro, $params = null)
  {
    ...
    switch ($macro)
    {
      ...
      case 'setglobalinfo':
        ...
        break;
      case 'setplaceholder':
        // $params[0] = placeholder name
        // $params[1] = placeholder value
        if (isset($params[0]))
        {
          if (isset($params[1]))
          {
            $placeholders[$params[0]] = $params[1];
          } else {
            unset($placeholders[$params[0]]);
          }
        }
        break;
      // get methods
      ...
      case 'getplaceholder':
        ...
        if (isset($placeholders[$params])) $value = $placeholders[$params];
        break;
      // macro methods
      case 'querybuild':
      case 'querybuildfull':
      case 'querybuildappend':
        ...
        if (strlen($value) > 0)
        {
          if ($macro === 'querybuildfull') { $value = '?' . $value; }
          if ($macro === 'querybuildappend') { $value = '&' . $value; }
        }
        break;
      case 'queryvar':
      ...
      case 'username':
        ...
        $value = ($user->get('guest') == 1) ? $params[0] : $user->get('username');
        break;
      case 'userid':
        $user = JFactory::getUser();
        $value = ($user->get('guest') == 1) ? $params[0] : $user->get('id');
        break;
      case 'tableselect':
      ...
      case 'substr':
        ...
        if ( (count($arg) == 1) || (count($arg) == 2) )
        {
          ...
          if ($value != '')
          {
            $value = (count($arg) == 1) ? substr($value, $arg[0]) : substr($value, $arg[0], $arg[1]);
          }
        }
        break;
      case 'strip_tags':
      ...
      case 'preg_placeholder':
        // $params[0] = N,placeholder
        // $params[1] = pattern
        // N: regexp pattern to return
        // placeholder: placeholder name
        preg_match("/^([^\,]+)\,(.*)$/iU", trim($params[0]), $arg);
        if (count($arg) == 3)
        {
          ...
          {
            if (@preg_match($params[1], $placeholders[$arg[2]], $matches) !== false)
            {
        ...
        break;
      case 'lowercase':
        $value = strtolower($params[0]);
        break;
      case 'uppercase':
        $value = strtoupper($params[0]);
        break;
      case 'urldecode':
        $value = urldecode($params[0]);
        break;
      case 'urlencode':
        $value = urlencode($params[0]);
        break;
      case 'rawurldecode':
        $value = rawurldecode($params[0]);
        break;
      case 'rawurlencode':
        $value = rawurlencode($params[0]);
        break;
      case 'str_replace':
        // $params[0] = 'search','replace'
        // $params[1] = input string
        $value = $params[1];
        $r = preg_match("/^'(.*)','(.*)'$/seuU", trim($params[0]), $arg);
        if ($r == 1)
        {
      ...
      if (strlen($row_value) > 0)
      {
        ...
        if ((strlen($parts_key) > 0) && (preg_match(PLACEHOLDERS_MATCH_MASK,$parts_key) === 1))
        {
            ...
            if (strlen($parts_value) > 0)
            {
                ...
                // Load placeholder
                $ph = array( 0 => $parts_key, 1 => $placeholders[$parts_key]);
                self::manageMacroParams('setplaceholder', $ph);
  ...
  private function setRedirect()
  {
    ...
    if ( count($items) > 0 )
    {
      ...
      if ( !empty($items[0]->tourl) )
      {
        ...
        // Replace macros
        $tourl = preg_replace($patterns, $replacements, $tourl);
